...
public class ObjectLayerService {
	...
	public void fillWithExampleData() throws DataObjectDaoException {
		...
		sgd.insertSegmentController(controller3);
	...
	public static void main(String[] args) throws DataObjectDaoException {
		...
		if (params.contains("test")) 
			main.test();
		else if (params.contains("fill")) 
			main.fillWithExampleData();
	}
	
	public void test() throws DataObjectDaoException {
		CabinetDao cd = getCabinetDao();
		testCabinets(cd);

		SegmentControllerDao sgd = getSegmentControllerDao();
		testSegmentControllers(sgd);

		FixturesDao fd = getFixturesDao();
		testFixtures(fd);

		DriverDao dd = getDriverDao();
		testDrivers(dd);

		ErrorDao ed = getErrorDao();
		testErrors(ed);

		SensorDao sensorDao = getSensorDao();
		testSensors(sensorDao);
